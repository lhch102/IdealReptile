...
def resultData(userName, password):
    ...
    if data['success']:
        ...
        selectUser = "select count(1) from user where user_name = %s" % (userName)
        count = connectionDataBase(selectUser, 'execute')[0][0]
        # 没有此用户，爬取本月甚至所有的考勤
        if count == 0:
            ...
            # 开始爬取数据
            responseIserCardInfo(0, cookies)
        else:
            # 有用户，则直接爬取今天的时间
            responseIserCardInfo(1, cookies)
